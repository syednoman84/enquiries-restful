...
public class HibernateSearchService {
    ...
    public List<Enquiry> hibernateSearch(String selector, String searchFor, int limit, String dateRangeString) {
        ...
        QueryBuilder queryBuilder = fullTextEntityManager.getSearchFactory().buildQueryBuilder().forEntity(Enquiry.class).get();
        ...
        @SuppressWarnings("unchecked") List<Enquiry> enquiryList = jpaQuery.getResultList();
        ...
        enquiryList = enquiryList.stream().filter(e -> e.getCreatedDate().isAfter(startDate)).filter(e -> e.getCreatedDate().isBefore(endDate)).collect(Collectors.toList());

        // limiting the results number
        if (limit < 1) limit = Integer.MAX_VALUE;
        enquiryList = enquiryList.stream().limit(limit).collect(Collectors.toList());
    ...
    public Query defaultQuery(String searchFor, QueryBuilder queryBuilder) {
        Query luceneQuery = queryBuilder.keyword().fuzzy().withEditDistanceUpTo(1).withPrefixLength(0).onFields("name", "email", "message", "phone", "isbn").matching(searchFor).createQuery();
        return luceneQuery;
    }

    public Query keywordExactQuery(String searchFor, QueryBuilder queryBuilder) {
        Query luceneQuery = queryBuilder.keyword().onField("name").boostedTo(8f).andField("email").boostedTo(6f).andField("message").boostedTo(4f).andField("phone").boostedTo(2f).andField("isbn").boostedTo(4f).matching(searchFor).createQuery();
        return luceneQuery;
    }

    public Query keywordFuzzy1Query(String searchFor, QueryBuilder queryBuilder) {
        Query luceneQuery = queryBuilder.keyword().fuzzy().withEditDistanceUpTo(1).withPrefixLength(0).onField("name").boostedTo(8f).andField("email").boostedTo(6f).andField("message").boostedTo(4f).andField("phone").boostedTo(2f).andField("isbn").boostedTo(4f).matching(searchFor).createQuery();
        return luceneQuery;
    }

    public Query keywordFuzzy2Query(String searchFor, QueryBuilder queryBuilder) {
        Query luceneQuery = queryBuilder.keyword().fuzzy().withEditDistanceUpTo(2).withPrefixLength(0).onField("name").boostedTo(8f).andField("email").boostedTo(6f).andField("message").boostedTo(4f).andField("phone").boostedTo(2f).andField("isbn").boostedTo(4f).matching(searchFor).createQuery();
        return luceneQuery;
    }

    public Query keywordWildcardQuery(String searchFor, QueryBuilder queryBuilder) {
        Query luceneQuery = queryBuilder.keyword().wildcard().onField("name").boostedTo(8f).andField("email").boostedTo(6f).andField("message").boostedTo(4f).andField("phone").boostedTo(2f).andField("isbn").boostedTo(4f).matching(searchFor).createQuery();
        return luceneQuery;
    }

    public Query phraseExactQuery(String searchFor, QueryBuilder queryBuilder) {
        Query luceneQuery = queryBuilder.phrase().withSlop(0).onField("name").boostedTo(8f).andField("email").boostedTo(6f).andField("message").boostedTo(4f).andField("phone").boostedTo(2f).andField("isbn").boostedTo(4f).sentence(searchFor).createQuery();
        return luceneQuery;
    }

    public Query phraseSlop1Query(String searchFor, QueryBuilder queryBuilder) {
        Query luceneQuery = queryBuilder.phrase().withSlop(1).onField("name").boostedTo(8f).andField("email").boostedTo(6f).andField("message").boostedTo(4f).andField("phone").boostedTo(2f).andField("isbn").boostedTo(4f).sentence(searchFor).createQuery();
        return luceneQuery;
    }

    public Query phraseSlop2Query(String searchFor, QueryBuilder queryBuilder) {
        Query luceneQuery = queryBuilder.phrase().withSlop(2).onField("name").boostedTo(8f).andField("email").boostedTo(6f).andField("message").boostedTo(4f).andField("phone").boostedTo(2f).andField("isbn").boostedTo(4f).sentence(searchFor).createQuery();
        return luceneQuery;
    }

    public Query phraseSlop3Query(String searchFor, QueryBuilder queryBuilder) {
        Query luceneQuery = queryBuilder.phrase().withSlop(3).onField("name").boostedTo(8f).andField("email").boostedTo(6f).andField("message").boostedTo(4f).andField("phone").boostedTo(2f).andField("isbn").boostedTo(4f).sentence(searchFor).createQuery();
        return luceneQuery;
    }

    public Query simpleQueryStringQuery(String searchFor, QueryBuilder queryBuilder) {
        Query luceneQuery = queryBuilder.simpleQueryString().onField("name").boostedTo(8f).andField("email").boostedTo(6f).andField("message").boostedTo(4f).andField("phone").boostedTo(2f).andField("isbn").boostedTo(4f).matching(searchFor).createQuery();
        return luceneQuery;
    }
